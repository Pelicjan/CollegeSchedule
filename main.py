...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.scraper = Scraper(self.init, self.group)
        self.download_data()

    def init(self, group):
        if group == '':
            print("To pierwsze uruchomienie! Trwa pobieranie danych z e-Dziekanatu.")
        else:
            self.group = group
    ...
    def get_week(self, number):
        if 1 <= number <= 22:
            ...
            self.blocks = self.scraper.get_week_blocks(self.group, self.week)
